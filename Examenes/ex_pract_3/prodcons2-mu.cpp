...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      {
        case 0:
          cout << "\tMODO 0" << endl << flush;
          emisor_establecido = false;
              // 1. determinar si puede enviar solo prod., solo cons, o todos
              if ( num_celdas_ocupadas == 0 ){               // si buffer vacío
                 id_emisor_aceptable = tag_prod ;       // $~~~$ solo prod.
                 emisor_establecido=true;
              }else if ( num_celdas_ocupadas == tam_vector ){ // si buffer lleno
                 id_emisor_aceptable = tag_cons ;      // $~~~$ solo cons.
                 emisor_establecido=true;
              }else{                                          // si no vacío ni lleno
                while(!emisor_establecido){
                 cout << "Buscando emisor....." << endl;
                  if(estado.MPI_TAG == tag_prod){
                    id_emisor_aceptable = tag_prod ;
                     emisor_establecido=true;
                  } else if(estado.MPI_TAG == tag_cons && estado.MPI_SOURCE == 1){
                    id_emisor_aceptable = tag_cons ;
                     emisor_establecido=true;
                  }
                }
              }
              ...
              switch( estado.MPI_TAG ) // leer emisor del mensaje en metadatos
              {
                 ...
                 case tag_cons: // si ha sido el consumidor: extraer y enviarle
                    if (estado.MPI_SOURCE == 0){
                      modo = 1;
                      cout << "Consumidor 0 ha sido rechazado. " << endl ;
                    } else{
        ...
        case 1:
          cout << "\tMODO 1" << endl << flush;
              ...
              if ( num_celdas_ocupadas == 0 ){               // si buffer vacío
                ...
                while(!emisor_establecido){
                  ...
                  if(estado.MPI_TAG == tag_prod){
                    id_emisor_aceptable = tag_prod ;
                     emisor_establecido=true;
                  } else if(estado.MPI_TAG == tag_cons && estado.MPI_SOURCE == 0){
                    id_emisor_aceptable = tag_cons ;
                     emisor_establecido=true;
                  }
                }
              ...
              switch( estado.MPI_TAG ) // leer emisor del mensaje en metadatos
              {
                 ...
                 case tag_cons: // si ha sido el consumidor: extraer y enviarle
                    if (estado.MPI_SOURCE == 1){
                      modo = 0;
                      cout << "Consumidor 1 ha sido rechazado. " << endl ;
                    } else{
                      valor = buffer[primera_ocupada] ;
                      primera_ocupada = (primera_ocupada+1) % tam_vector ;
                      num_celdas_ocupadas-- ;
